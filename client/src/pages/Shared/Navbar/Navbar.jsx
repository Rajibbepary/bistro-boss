...
const [open, setOpen] = useState(false)
  ...
  <div className="fixed w-full z-10 bg-black/30 ">
    ...
    className={`flex items-center justify-between px-4 md:px-12 lg:px-12 xl:px-24 py-4 text-[#FFFFFF] relative transition-all ${location.pathname ==='/' && 'bg-light' }`}>
        <Link to='/'>
          <img src={assets.logo} className="w-10 max-sm:hidden h-10 object-cover" alt=""  />
    ...
    >
      {link.name}
        ...
        <div className="flex max-sm:flex-col items-start sm:items-center gap-6">
            ...
            <button onClick={()=> {user ? logOut() : handleLogOut(true)}} className="cursor-pointer px-8 py-2 bg-primary hover:bg-primary-dull transition-all text-white rounded-lg">{user ? 'Logout' : 'Login'}</button>
        </div>
      </div>
